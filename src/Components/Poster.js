...
const Poster = ({id, idx, imageUrl, title, rating, year, isMovie = false}) => {
  return (
    <Link to={isMovie ? `/movie/${id}` : `/show/${id}`}>
      <Container trs={Math.random()} idx={idx} className="title_ref">
        ...
        <Title className="title_ref">{title.length > 18 ? `${title.substring(0, 15)}...` : title}</Title>
        <Year>{year}</Year>
      </Container>
    </Link>
  )
